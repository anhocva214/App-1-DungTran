...
const NoACtionPage = ({_text, set_text, set_statusPage, navigation}) => {
    const handleLogin = () => {
        if (_text.trim() == "1") {
            set_statusPage(1);
            setTimeout(() => {
...
const LoginPage = ({ navigation }) => {
    ...
    const ShowSection = () => {
        ...
        return <NoACtionPage _text={_text} set_text={(text)=>set_text(text)} set_statusPage={(status)=>set_statusPage(status)} navigation={navigation}/>
    }
